...
namespace sandbox
{
  ...
  class MemoryServiceProvider
  {
    ...
    std::unordered_map<
      platform::handle_t,
      std::pair<platform::Handle, SharedMemoryProvider*>>
      ranges;
    /**
    ...
    bool validate_and_insert(
      SharedMemoryProvider* s, void* address, snmalloc::MetaEntry m)
    {
      size_t range_size;
  ...
  public:
     ...
     */
    void
    add_range(SharedMemoryProvider& memory_provider, platform::Handle&& socket)
    {
      {
        std::lock_guard g(m);
        register_fd(socket);
